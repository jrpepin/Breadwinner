...
earndat <- earndat %>%
  select(1, 735:792) # This needs to be updated each time new variables are added/created.
...
earndat <- earndat %>%
  mutate(
    ...
    bwf5_m4 = case_when(
      ...
      TRUE                                   ~  NA_character_),
    bwf5_m5 = case_when(
      (earndat$famearn_m5/earndat$momearn_m5) > .5 ~ "Breadwinner",
      (earndat$famearn_m5/earndat$momearn_m5) < .5 ~ "Not a breadwinner",
      TRUE                                   ~  NA_character_),
  )
